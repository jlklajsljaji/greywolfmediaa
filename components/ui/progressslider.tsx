...
export default function ProgressSlider({ items }: { items: Item[] }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto text-center">
          {/* <h1 className="heading max-w-7xl mb-10 pl-4 mx-auto text-xl md:text-5xl font-bold text-neutral-800 dark:text-neutral-200 font-sans">
      ...
      {/* Buttons */}
      <div className="max-w-xs sm:max-w-sm md:max-w-3xl mx-auto flex justify-center items-center gap-4 mt-8">
          ...
          >
            <span className={`text-center flex flex-col items-center ${active === index ? '' : 'opacity-50 group-hover:opacity-100 group-focus:opacity-100 transition-opacity'}`}>
              <span className="flex items-center justify-center relative w-9 h-9 rounded-full bg-indigo-100 mb-2">
                <Image src={item.buttonIcon} alt={item.desc} width={10} height={10}/>
              </span>
              <span className="block text-sm font-medium text-white-200 mb-2">{item.desc}</span>
